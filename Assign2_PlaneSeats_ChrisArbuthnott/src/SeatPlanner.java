...
public class SeatPlanner {
	...
	// String representations of the row and column preferences.
	...
	public void start(int numRows, int numCols)
	{
		...
		while(anotherPassenger) // loop to book passengers.
		{	
			...
			if (seat[0] == -1) // no suitable seat found.
			{
		...
		}// end while-loop to book passengers
	...
	// toggles column or row preference.
	private int otherChoice(int num)
	{
		if (num == 1)
			...
			return 1;
	}// end otherChoice method
	
	// Finds an available seat matching row and column preferences.
	// If none is available, user is prompted with other options, and best match
	// possible is returned.  {-1,-1} returned for no match on this flight.
	private int[] getBestSeat(int rowPref, int colPref)
	{
		String inputYN, outputString;
		int negArray[] = {-1,-1};
		
		// Check for first choice seats
		if (flight.hasMatch(rowPref, colPref))
			return flight.getMatch(rowPref, colPref);
			
		// Check for seats in alternate WINDOW/AISLE
		...
		// Check for seats in alternate class.
		...
		// Check for seats in alternate class and WINDOW/AISLE
	...
	// Output seat information to the console.
	...
	// Collect a validated y or n, prompting user with input message.
	private String collectYN(String message)
	{
		...
		return inputYN;
	}// end collectYN method
	
	// Collect a validated 1 or 2, prompting user with input message.
	private String collect12(String message)
	{
		...
		return inputNum;
	}// end collect12 method
	
	// Collect a validated name, prompting user with input message.
